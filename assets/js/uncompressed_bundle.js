...
window.onload = function(){
  if (otherOnload) otherOnload();
    ...
    if (document.documentMode || /Edge/.test(navigator.userAgent)) {
        var images_covers = document.getElementsByClassName('featured-box-img-cover');
        var appendedString = "";
        for (var i = 0, len = images_covers.length; i < len; i++) {
            var item = images_covers[i];
            appendedString += 
                '<div style="display: none; background-size:cover;background-repeat: no-repeat; background-position: 50% 20%; height: 290px; background-image: '
                +'url(' + item.getAttribute('src') + ')'
                +';"></div>';
        }
        images_covers.innerHTML = appendedString;
    }
    
    // Navigation bar Initialization and Functionality
    var HIDE_SCROLL_POS = 100;
    window.onscroll = function() {
      var currentScrollPos = window.scrollY || window.scrollTop || document.getElementsByTagName("html")[0].scrollTop;
      if (HIDE_SCROLL_POS > currentScrollPos) {
